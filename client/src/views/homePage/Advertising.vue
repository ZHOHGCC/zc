<template>
  <el-main>
    ...
    <!-- 添加 活动-->
    <add-adv :add="add"
             @updata='getList'></add-adv>
    <change-adv :change="change"
                @updata='getList'></change-adv>
    <!-- 删除提示框 -->
...
<script>
import addAdv from './visible/addAdv'
import changeAdv from './visible/changeAdv'
export default {
  name: 'basetable',
  data () {
    return {
  ...
  methods: {
    ...
    // -------------------------------------------------------------------------------
    getList () {
      this.$axios.get("/home/adv").then(res => {
    ...
    deleteRow (row) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        let id = row.id
        this.$axios.delete(`/home/adv/${id}`, {          params: {
            id: id
          }        }).then(() => {
          this.$message({
            type: 'success',
